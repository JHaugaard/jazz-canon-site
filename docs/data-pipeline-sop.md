...
- Full playback (Step 3) uses a separate *client-side* developer token baked
  into the build (`PUBLIC_APPLE_DEV_TOKEN`), generated by
  `scripts/gen_dev_token.py`. It has a ~6-month max lifetime — **rotation is a
  recurring chore**, fully documented in **[apple-token-rotation.md](apple-token-rotation.md)**.
